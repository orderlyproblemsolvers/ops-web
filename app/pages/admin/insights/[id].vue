    ...
    <div v-else>
      <div class="flex items-center justify-between mb-8">
        <div>
          ...
          <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Insight</h1>
        </div>
        <div class="flex items-center gap-3">
          <UButton color="secondary" @click="savePost(true)" :loading="isSaving">Publish Updates</UButton>
        </div>
      </div>
      ...
      <div class="space-y-6">
        ...
        <UFormField label="Cover Image">
          ...
          >
            <div v-if="isUploadingCover" class="flex flex-col items-center">
              <UIcon name="i-lucide-loader-circle" class="w-6 h-6 animate-spin text-gray-500" />
              <span class="text-sm mt-2 text-gray-500 font-medium">Uploading...</span>
            </div>
            <div v-else class="flex flex-col items-center">
              <UIcon name="i-lucide-image-plus" class="w-8 h-8 text-gray-400 mb-2" />
              <span class="text-sm text-gray-600 dark:text-gray-400 font-medium">Click to upload cover image</span>
            </div>
          </div>
          <input ref="coverFileInput" type="file" accept="image/*" class="hidden" @change="handleCoverImageUpload" />
        </UFormField>

        <div class="grid grid-cols-2 gap-4">
          <UFormField label="Category">
            <USelectMenu :ui="{base: 'bg-primary', content: 'bg-primary'}" v-model="post.category" :items="['Strategy', 'Engineering', 'Data & AI', 'Design', 'Company']" />
          </UFormField>
          <UFormField label="Format">
            <USelectMenu :ui="{base: 'bg-primary', content: 'bg-primary'}" v-model="post.format" :items="['Article', 'Guide', 'Report']" />
          </UFormField>
        </div>
        ...
        <div class="min-w-0 w-full border border-gray-200 dark:border-gray-800 rounded-xl bg-white dark:bg-gray-900 shadow-sm focus-within:ring-2 focus-within:ring-accent/20 transition-all text-gray-950 dark:text-gray-100">
          <UEditor
            ...
            :placeholder="{ placeholder: 'Start writing your insight...', mode: 'firstLine' }"
            :ui="{ base: 'bg-ops-black custom-scrollbar focus:outline-none min-h-[400px] sm:min-h-[500px] max-h-[700px] overflow-y-auto max-w-none p-4 sm:p-6' }"
          >
            <div 
              class="min-w-0 border-b border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 sticky top-0 z-20 shadow-sm rounded-t-xl"
              :class="showTableMenu ? 'overflow-visible' : 'overflow-x-auto'"
            >
              <div class="flex items-center gap-1 p-2 min-h-10.5 w-max min-w-full bg-ops-black">
                <UEditorToolbar :ui="{base: 'bg-ops-black'}" :editor="editor" :items="toolbarItems" />

                <div class="toolbar-color-btn relative flex items-center justify-center p-2 rounded cursor-pointer shrink-0" title="Text Color">
                  <span class="font-bold text-xs select-none dark:text-white" :style="{ color: editor.getAttributes('textStyle').color || '' }">A</span>
                  <input
                    type="color"
                ...
                <select
                  class="shrink-0 h-8 text-xs text-gray-800 dark:text-gray-200 border border-gray-200 dark:border-gray-700 rounded bg-white dark:bg-gray-800 px-1 outline-none hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer"
                ...
                <div class="w-px h-5 bg-gray-200 dark:bg-gray-700 shrink-0 mx-1" />

                <div class="relative shrink-0">
                  <div v-if="showTableMenu" class="fixed inset-0 z-30 cursor-default" @click.stop="closeTableMenu"></div>

                  <button
                    type="button"
                    title="Table options"
                    class="table-trigger relative z-40 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-200"
                  ...
                  >
                    ...
                    <span class="i-lucide-chevron-down w-3 h-3 transition-transform duration-150" :class="{ 'rotate-180': showTableMenu }" />
                  </button>

                  <div v-if="showTableMenu" class="table-panel z-40 dark:bg-gray-800 dark:border-gray-700">
                    <div class="table-panel-section">
                      <p class="table-panel-heading">Insert</p>
                      <button type="button" class="table-action dark:hover:bg-gray-700" @click="customHandlers.insertTable.execute(editor)">
                        <span class="table-action-icon bg-blue-50 text-blue-600 dark:bg-blue-500/10 dark:text-blue-400">
                          <span class="i-lucide-table-2 w-3.5 h-3.5" />
                        </span>
                        <span class="flex flex-col items-start">
                          <strong class="text-gray-900 dark:text-white font-medium">New table</strong>
                    ...
                    <template v-if="editor.isActive('table')">
                      <div class="table-panel-divider dark:bg-gray-700" />
                      <div class="table-panel-section">
                        <p class="table-panel-heading">Columns</p>
                        <div class="table-icon-row">
                          <button type="button" class="table-icon-btn dark:bg-gray-900 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-700" title="Add column before" @click="customHandlers.addColumnBefore.execute(editor)">
                            <span class="i-lucide-arrow-left-to-line w-4 h-4" />
                            <span class="table-icon-label">Before</span>
                          </button>
                          <button type="button" class="table-icon-btn dark:bg-gray-900 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-700" title="Add column after" @click="customHandlers.addColumnAfter.execute(editor)">
                            <span class="i-lucide-arrow-right-to-line w-4 h-4" />
                            <span class="table-icon-label">After</span>
                          </button>
                          <button type="button" class="table-icon-btn table-icon-btn--danger dark:bg-red-500/10 dark:border-red-500/20" title="Delete column" @click="customHandlers.deleteColumn.execute(editor)">
                      ...
                      <div class="table-panel-section">
                        ...
                        <div class="table-icon-row">
                          <button type="button" class="table-icon-btn dark:bg-gray-900 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-700" title="Add row above" @click="customHandlers.addRowBefore.execute(editor)">
                            <span class="i-lucide-arrow-up-to-line w-4 h-4" />
                            <span class="table-icon-label">Above</span>
                          </button>
                          <button type="button" class="table-icon-btn dark:bg-gray-900 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-700" title="Add row below" @click="customHandlers.addRowAfter.execute(editor)">
                            <span class="i-lucide-arrow-down-to-line w-4 h-4" />
                            <span class="table-icon-label">Below</span>
                          </button>
                          <button type="button" class="table-icon-btn table-icon-btn--danger dark:bg-red-500/10 dark:border-red-500/20" title="Delete row" @click="customHandlers.deleteRow.execute(editor)">
                            <span class="i-lucide-trash-2 w-4 h-4" />
                            <span class="table-icon-label">Delete</span>
                          </button>
                        </div>
                      ...
                      <div class="table-panel-section">
                        ...
                        <div class="table-icon-row">
                          <button type="button" class="table-icon-btn dark:bg-gray-900 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-700" title="Merge selected cells" @click="customHandlers.mergeCells.execute(editor)">
                            <span class="i-lucide-merge w-4 h-4" />
                            <span class="table-icon-label">Merge</span>
                          </button>
                          <button type="button" class="table-icon-btn dark:bg-gray-900 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-700" title="Split merged cell" @click="customHandlers.splitCell.execute(editor)">
                      ...
                      <div class="table-panel-divider dark:bg-gray-700" />
                      <div class="table-panel-section">
                        <button type="button" class="table-action table-action--danger dark:hover:bg-red-500/10" @click="customHandlers.deleteTable.execute(editor)">
                          <span class="table-action-icon bg-red-50 text-red-500 dark:bg-red-500/20">
  ...
  <Teleport to="body">
    <Transition name="yt-dialog">
      <div v-if="showYoutubeDialog" class="yt-overlay" @click.self="closeYoutubeDialog">
        <div class="yt-dialog dark:bg-gray-900" role="dialog" aria-modal="true" aria-label="Embed YouTube video">
          <div class="yt-dialog-header">
            <div class="yt-dialog-icon dark:bg-red-500/10">
              <span class="i-lucide-youtube w-5 h-5 text-red-500" />
            </div>
            <div>
              <p class="yt-dialog-title dark:text-white">Embed YouTube video</p>
          ...
          <div class="yt-dialog-body">
            <label class="yt-label dark:text-gray-300" for="yt-url-input">Embed code or URL</label>
            <textarea
              id="yt-url-input"
              v-model="youtubeUrlInput"
              class="yt-input yt-textarea dark:bg-gray-800 dark:border-gray-700 dark:text-white"
          ...
          <div class="yt-dialog-footer dark:border-gray-800">
            <button type="button" class="yt-btn-cancel dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:bg-gray-700" @click="closeYoutubeDialog">Cancel</button>
            <button type="button" class="yt-btn-embed bg-accent hover:bg-accent-hover" @click="commitYoutubeEmbed">
...
useSeoMeta({ title: 'Edit Insight | OPS Admin' })
...
const { data: insightData, status: fetchStatus } = await useFetch(`/api/insights/${route.params.id}`)

if (insightData.value) {
  post.value = { ...insightData.value } as any
}

// --- SAVE / UPDATE LOGIC ---
async function savePost(publish: boolean) {
  if (!post.value.title || !post.value.content) {
    toast.add({ title: 'Missing Fields', description: 'Title and content are required.', color: 'error' })
    return
  }
  isSaving.value = true
  try {
    // Note: We use PUT and point to the specific ID
    await $fetch(`/api/insights/${route.params.id}`, {
      method: 'PUT',
      body: { ...post.value, isPublished: publish }
    })
    toast.add({ title: publish ? 'Changes Published!' : 'Draft Updated!', color: 'success' })
    await navigateTo('/admin/insights')
  } catch (error: any) {
    toast.add({ title: 'Error updating post', description: error.message, color: 'error' })
  } finally {
...
const CLOUD_NAME = (config.public.cloudinaryCloudName as string) || ''
const UPLOAD_PRESET = (config.public.cloudinaryUploadPreset as string) || ''
...
const handleCoverImageUpload = async (event: Event) => {
  ...
  if (!CLOUD_NAME) {
    toast.add({ title: 'Upload Failed', description: 'Cloudinary config is missing', color: 'error' })
    return
  }
  ...
  try {
    ...
    if (data.secure_url) {
      post.value.coverImage = data.secure_url
    }
  } catch (e) {
    toast.add({ title: 'Upload Failed', description: 'Could not upload cover image.', color: 'error' })
  } finally {
...
const IframeNode = Node.create({
  ...
  renderHTML({ HTMLAttributes }) {
    return [
      ...
      ['iframe', { ...HTMLAttributes, referrerpolicy: 'strict-origin-when-cross-origin', allowfullscreen: '' }],
    ]
  },
  addCommands() {
    return { insertIframe: (attrs: Record<string, any>) => ({ commands }: any) => commands.insertContent({ type: 'iframe', attrs }) } as any
  },
...
function closeYoutubeDialog() { showYoutubeDialog.value = false }

function commitYoutubeEmbed() {
  const raw = youtubeUrlInput.value.trim()
  if (!raw) { youtubeUrlError.value = 'Please enter an embed code or URL.'; return }
  const editor = editorRef.value?.editor
  if (!editor) return

  if (raw.startsWith('<iframe') || raw.startsWith('<IFRAME')) {
    const src = extractIframeSrc(raw)
    if (!src) { youtubeUrlError.value = 'Could not find a src URL in that embed code.'; return }
  ...
  if (!videoId) { youtubeUrlError.value = 'Paste the YouTube <iframe> embed code, or a URL.'; return }
...
function closeTableMenu() { showTableMenu.value = false }

const customHandlers = {
  imageUpload: {
    canExecute: () => !isUploading.value,
    execute: (editor: Editor) => { fileInput.value?.click(); return editor.chain() },
    isActive: () => false,
    isDisabled: () => isUploading.value,
  },
  insertTable: {
    canExecute: (editor: Editor) => !editor.isActive('table'),
    execute: (editor: Editor) => { editor.chain().focus().insertTable({ rows: 3, cols: 3, withHeaderRow: true }).run(); closeTableMenu(); return editor.chain() },
    isActive: () => false,
    isDisabled: (editor: Editor) => editor.isActive('table'),
  },
  addColumnBefore: { canExecute: (editor: Editor) => editor.isActive('table'), execute: (editor: Editor) => { editor.chain().focus().addColumnBefore().run(); closeTableMenu(); return editor.chain() }, isActive: () => false, isDisabled: (editor: Editor) => !editor.isActive('table') },
  addColumnAfter: { canExecute: (editor: Editor) => editor.isActive('table'), execute: (editor: Editor) => { editor.chain().focus().addColumnAfter().run(); closeTableMenu(); return editor.chain() }, isActive: () => false, isDisabled: (editor: Editor) => !editor.isActive('table') },
  deleteColumn: { canExecute: (editor: Editor) => editor.isActive('table'), execute: (editor: Editor) => { editor.chain().focus().deleteColumn().run(); closeTableMenu(); return editor.chain() }, isActive: () => false, isDisabled: (editor: Editor) => !editor.isActive('table') },
  addRowBefore: { canExecute: (editor: Editor) => editor.isActive('table'), execute: (editor: Editor) => { editor.chain().focus().addRowBefore().run(); closeTableMenu(); return editor.chain() }, isActive: () => false, isDisabled: (editor: Editor) => !editor.isActive('table') },
  addRowAfter: { canExecute: (editor: Editor) => editor.isActive('table'), execute: (editor: Editor) => { editor.chain().focus().addRowAfter().run(); closeTableMenu(); return editor.chain() }, isActive: () => false, isDisabled: (editor: Editor) => !editor.isActive('table') },
  deleteRow: { canExecute: (editor: Editor) => editor.isActive('table'), execute: (editor: Editor) => { editor.chain().focus().deleteRow().run(); closeTableMenu(); return editor.chain() }, isActive: () => false, isDisabled: (editor: Editor) => !editor.isActive('table') },
  deleteTable: { canExecute: (editor: Editor) => editor.isActive('table'), execute: (editor: Editor) => { editor.chain().focus().deleteTable().run(); closeTableMenu(); return editor.chain() }, isActive: () => false, isDisabled: (editor: Editor) => !editor.isActive('table') },
  mergeCells: { canExecute: (editor: Editor) => editor.isActive('table'), execute: (editor: Editor) => { editor.chain().focus().mergeCells().run(); closeTableMenu(); return editor.chain() }, isActive: () => false, isDisabled: (editor: Editor) => !editor.isActive('table') },
  splitCell: { canExecute: (editor: Editor) => editor.isActive('table'), execute: (editor: Editor) => { editor.chain().focus().splitCell().run(); closeTableMenu(); return editor.chain() }, isActive: () => false, isDisabled: (editor: Editor) => !editor.isActive('table') },
  insertYoutube: { canExecute: () => true, execute: (editor: Editor) => { openYoutubeDialog(); return editor.chain() }, isActive: () => false, isDisabled: () => false },
...
const handleFile = async (event: Event) => {
  ...
  if (!target.files?.length) return
  const editor = editorRef.value?.editor
  if (!editor || !CLOUD_NAME) {
    if (!CLOUD_NAME) toast.add({ title: 'Upload Failed', description: 'Cloudinary config is missing', color: 'error' })
    return
  }
...
const toolbarItems = computed(() => [
  ...
  [
    ...
    { kind: 'imageUpload', icon: isUploading.value ? 'i-lucide-loader-circle' : 'i-lucide-image' },
...
<style scoped>
/* Toolbar Theming to match Nuxt UI & Dark mode */
:deep([role="toolbar"] button) { background-color: transparent; }
:deep([role="toolbar"] button:hover) { background-color: rgba(0,0,0,0.05); }
:deep([role="toolbar"] button[data-active="true"]),
:deep([role="toolbar"] button[aria-pressed="true"]) { background-color: #111111; color: #ffffff; }
:deep(.dark [role="toolbar"] button:hover) { background-color: rgba(255,255,255,0.1); }
:deep(.dark [role="toolbar"] button[data-active="true"]),
:deep(.dark [role="toolbar"] button[aria-pressed="true"]) { background-color: rgba(255,255,255,0.2); color: #ffffff; }

.toolbar-color-btn:hover { background-color: rgba(0,0,0,0.05); }
:deep(.dark) .toolbar-color-btn:hover { background-color: rgba(255,255,255,0.1); }
...
.yt-btn-embed { display: flex; align-items: center; gap: 0.375rem; height: 2.25rem; padding: 0 1rem; font-size: 0.8125rem; font-weight: 600; color: white; border: none; border-radius: 0.5rem; cursor: pointer; transition: background 0.1s; }
...
:deep(.tiptap th) { background-color: #f9fafb; font-weight: 600; font-size: 0.875rem; text-align: left; }
:deep(.dark .tiptap td), :deep(.dark .tiptap th) { border-color: #374151; }
:deep(.dark .tiptap th) { background-color: #1f2937; color: #f3f4f6; }
:deep(.tiptap .selectedCell) { background-color: rgba(59, 130, 246, 0.1); }
...
:deep(.tiptap h2) { font-size: 1.5rem; font-weight: 700; margin-top: 1.5rem; margin-bottom: 0.75rem; line-height: 1.3; }
:deep(.tiptap h3) { font-size: 1.25rem; font-weight: 600; margin-top: 1.25rem; margin-bottom: 0.5rem; line-height: 1.3; }
:deep(.tiptap p) { margin-bottom: 0.75rem; line-height: 1.6; }
:deep(.tiptap ul) { list-style-type: disc; padding-left: 1.5rem; margin-bottom: 0.75rem; }
:deep(.tiptap ol) { list-style-type: decimal; padding-left: 1.5rem; margin-bottom: 0.75rem; }
...
:deep(.dark .tiptap blockquote) { border-color: #4b5563; color: #9ca3af; }
