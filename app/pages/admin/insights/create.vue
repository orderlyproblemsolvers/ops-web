...
useSeoMeta({ title: 'Create Insight | OPS Admin' })
...
async function savePost(publish: boolean) {
  ...
  try {
    ...
    toast.add({ title: publish ? 'Published!' : 'Draft Saved!', color: 'success' })
    await navigateTo('/admin/insights')
  } catch (error: any) {
    toast.add({ title: 'Error saving post', description: error.message, color: 'error' })
  } finally {
